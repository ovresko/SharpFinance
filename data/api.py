...
class BaseClient(object):
    ...
    def _login_decorator(self, func, *args, **kwargs):
        ...
        try:
            ...
            #print(f"_login_decorator error {res}")
            return res
...
class Transaction(object):
    def __init__(self, trans_dict):
        ...
        self.tp = trans_dict['tp'] or 0
        self.sl = trans_dict['sl'] or 0
        #LOGGER.debug(f"Transaction {self.order_id} inited")


class Client(BaseClient):

    _days = {1:"Samedi",2:"Mardi",3:"Mercredi",4:"Jeudi",5:"Vendredi",6:"Samedi",7:"Dimanche"}
    ...
    def check_if_market_open(self, list_of_symbols,offset = 0):
        """check if market is open for symbol in symbols"""
        _td = datetime.today()
        _td += timedelta(hours=offset)
    ...
    def get_day(self,day_code):
        if not day_code or day_code not in self._days.keys():
            raise TypeError("day code invalide")
        if not isinstance(day_code, int):
            day_code = int(day_code)
        return self._days[day_code]
    ...
    def open_trade(self, mode, symbol, volume, take_profit=0, stop_loss=0,trunc=5,price=0,expiration=None,offset=0):
        """open trade transaction"""
        if mode in [MODES.BUY.value, MODES.SELL.value, MODES.BUY_LIMIT.value,MODES.BUY_STOP.value,MODES.SELL_LIMIT.value,MODES.SELL_STOP.value]:
        ...
        conversion_mode = {MODES.BUY.value: 'ask',MODES.BUY_STOP.value: 'ask',MODES.BUY_LIMIT.value: 'ask', MODES.SELL.value: 'bid', MODES.SELL_STOP.value: 'bid', MODES.SELL_LIMIT.value: 'bid'}
        if not price:
            price = self.get_symbol(symbol)[conversion_mode[mode]]
        # sl = self.truncate(price * (1 - (stop_loss/100)),decimals=trunc)
        # tp = self.truncate(price * (1 + (take_profit/100)),decimals=trunc)
        #self.LOGGER.debug(f"opening trade of {symbol} of {volume} price: {price} sl: {sl} tp: {tp} with "
        #                  f"{mode_name}")
        # symbol, mode, trans_type, volume, stop_loss=0, take_profit=0, **kwargs)
        response = self.trade_transaction(symbol, mode, 0, volume,stop_loss=stop_loss,take_profit=take_profit, price=price,expiration=expiration,offset=offset)
        self.update_trades()
    ...
    def _close_trade_only(self, order_id,trans_type = 2):
        """faster but less secure"""
        trade = self.trade_rec[order_id]

        #self.LOGGER.debug(f"closing trade {order_id}")
        try:
            #mode = { 'buy': 0, 'sell':1 , 'buy_limit':2 , 'buy_stop':4, 'sell_limi':3, 'sell_stop':5}[trade.mode]
            mode = trade.mode
            print(f"CLOSING {trade.symbol} {order_id} {trade.volume} {trade.mode} send mode {mode}")
            #mode = 0 if trade.mode == "buy" else 1
            response = self.trade_transaction(
                trade.symbol, mode, trans_type, trade.volume, order=trade.order_id,
                price=trade.price)
    ...
    def close_trade(self, trans,trans_type = 2):
        ...
        if isinstance(trans, Transaction):
            ...
            order_id = trans
        self.update_trades()
        return self._close_trade_only(order_id,trans_type=trans_type)
